...
- **Git 提交**：审查明确范围、运行必要验证并生成受控提交；有关联 Tower 时，评论始终
  先预览，确认后才发布。
- **GitLab 拉分支**：建 Issue，从该线 master 只建远端分支再本机 checkout。只适用于 GitLab。
- **GitLab 合并**：在确认后推送当前分支，通过 `sw merge` 合并到测试分支并追踪对应流水线；
  主干合并始终留给人工审核。只适用于 GitLab。
- **GitLab 解冲突**：合进 test 线撞冲突时在本机 `merge --no-ff`，问清谁改再 push。只适用于 GitLab。
- **日报**：只在明确要求时记录已经完成的代码工作，并按天汇总多个项目。
- **知识库初始化**：在业务仓库根建立 `.knowledge`。Umi/Antd 与 Taro 对着入口表填卡，
  其它项目摸代码建库。正文跟代码一起提交，不进插件。
- **对照知识库写计划**：查阅当前仓库 `.knowledge`，必要时在本机缓存写出开发计划（不进用户仓库）；
  主对话把知识对照进开发时派一次子任务，不把原文灌进长对话。
- **沉淀项目知识库**：开发完成后在当前会话里纠偏或新增 `.knowledge` 卡片，直接写入；
  不重新建库，不改业务代码。建议提交前先沉淀，让知识和代码同一批走。
...
- 打开系统 Skill：软链到该宿主的 skills 目录。宿主里看到的名字统一带 `spec-` 前缀
  （比如 `spec-git-commit`），跟你已有的同名 Skill 不会互相覆盖
- 自定义 Skill：在 Skills 页「自定义」里新增自己的目录。App 只往三个宿主做分发，
  不拷贝、不改你的文件。宿主里用目录原名，不加 `spec-` 前缀。删除前需要先关掉所有平台。
